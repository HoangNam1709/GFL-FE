...
export default function VehicleRegistrationPage() {
  ...
  const [formData, setFormData] = useState({ licensePlate: '', ticketType: 'Thang', ownerName: '', ownerId: '', ownerPhone: '', notes: '' });
  ...
  const handleChange = (field: string) => (e: ChangeEvent<HTMLInputElement>) => {
    setFormData({ ...formData, [field]: field === 'licensePlate' ? e.target.value.toUpperCase() : e.target.value });
        ...
        >
          {imgState.preview ? (
            <>
              ...
              <IconButton size="small" onClick={(e) => { e.stopPropagation(); handleRemoveImage(type); }} sx={{ position: 'absolute', top: 2, right: 2, bgcolor: 'rgba(0,0,0,0.6)', color: '#fff' }}><CancelIcon fontSize="small" /></IconButton>
            </>
          ) : (
  ...
  return (
    <Box sx={{ bgcolor: theme.palette.customBg?.main || 'background.default', minHeight: '100vh', p: { xs: 2, sm: 4 } }}>
      ...
      <Box sx={{ mb: 4, pb: 2, borderBottom: `2px solid ${theme.palette.divider}`, display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Box>
          <Typography variant="h5" sx={{ color: theme.palette.primary.main, fontWeight: 'bold' }}>HỆ THỐNG CẤP PHÁT & ĐĂNG KÝ VÉ XE ĐỊNH KỲ</Typography>
        ...
        <Box component="form" onSubmit={handleSubmit} noValidate>
          <Grid container spacing={3} sx={{ justifyContent: 'center' }}>
            <Grid size={{ xs: 12, md: 6 }}>
              <Card sx={{ border: `1px solid ${theme.palette.divider}`, borderRadius: '12px', height: '100%' }}>
                <CardContent sx={{ p: 3 }}>
                  <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}><PersonIcon color="primary" /> Thông Tin Chủ Xe / Tài Xế</Typography>
                  <Divider sx={{ mb: 3 }} />
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12 }}><TextField fullWidth label="Họ và Tên Chủ Xe" value={formData.ownerName} onChange={handleChange('ownerName')} slotProps={{ htmlInput: { style: { textTransform: 'uppercase' } } }} /></Grid>
                    <Grid size={{ xs: 12, sm: 6 }}><TextField fullWidth label="Số CCCD / Định danh" value={formData.ownerId} onChange={handleChange('ownerId')} slotProps={{ input: { startAdornment: <InputAdornment position="start"><BadgeIcon color="action" /></InputAdornment> } }} /></Grid>
                    <Grid size={{ xs: 12, sm: 6 }}><TextField fullWidth label="Số Điện Thoại" value={formData.ownerPhone} onChange={handleChange('ownerPhone')} slotProps={{ input: { startAdornment: <InputAdornment position="start"><PhoneIcon color="action" /></InputAdornment> } }} /></Grid>
                  </Grid>
                </CardContent>
              </Card>
            </Grid>
            ...
            <Grid size={{ xs: 12, md: 6 }}>
              <Card sx={{ border: `1px solid ${theme.palette.divider}`, borderRadius: '12px', height: '100%' }}>
                <CardContent sx={{ p: 3 }}>
                  <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}><DirectionsCarIcon color="primary" /> Thông Tin Phương Tiện & Hình Ảnh</Typography>
                  <Divider sx={{ mb: 3 }} />
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12 }}><TextField required fullWidth label="Biển Số Xe" value={formData.licensePlate} onChange={handleChange('licensePlate')} error={Boolean(errors.licensePlate)} helperText={errors.licensePlate} /></Grid>
            ...
            <Grid size={{ xs: 12, md: 6 }}><TextField fullWidth multiline rows={3} label="Ghi chú thêm..." value={formData.notes} onChange={handleChange('notes')} /></Grid>
            <Grid size={{ xs: 12, md: 6 }} sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end' }}>
              <CustomButton type="submit" variant="contained" size="large" startIcon={<SaveIcon />} isLoading={isLoading} sx={{ fontWeight: 'bold', px: 5, py: 1.8 }}>HOÀN TẤT ĐĂNG KÝ XE</CustomButton>
            </Grid>
          </Grid>
        </Box>
      ) : (
