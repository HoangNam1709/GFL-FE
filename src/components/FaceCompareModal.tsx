  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3, mt: 1 }}>
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <TextField label="Ngày sinh" fullWidth value={vehicleData?.birth || ''} slotProps={{ input: { readOnly: true } }} />
            <TextField label="Nơi thường trú" fullWidth multiline rows={2} value={vehicleData?.place || ''} slotProps={{ input: { readOnly: true } }} />

            {compareResult && (
              <Box sx={{ p: 2, bgcolor: 'action.hover', borderRadius: 1, border: '1px solid #ccc' }}>
                <Typography variant="subtitle2" color="success.main" sx={{ fontWeight: 'bold' }}>Kết quả đối sánh từ Server:</Typography>
                <Typography variant="body2">- Tỷ lệ trùng khớp: {compareResult?.data?.compare?.score ? `${(compareResult.data.compare.score * 100).toFixed(1)}%` : 'N/A'}</Typography>
      ...
      <DialogActions sx={{ p: 2, gap: 1 }}>
        <Button variant="outlined" color="inherit" onClick={onClose}>ĐÓNG MÀN HÌNH</Button>
        <Button variant="contained" color="primary" startIcon={isComparing ? <CircularProgress size={20} color="inherit" /> : <VerifiedUserIcon />} onClick={handleCompareFaces} disabled={isComparing} sx={{ color: '#ffffff !important' }}>
