    ...
    if (error.response) {
      ...
      switch (status) {
        case 401:
          ...
          localStorage.removeItem('accessToken'); // Xóa token cũ nhiễm độc
        ...
        default:
          console.error(`Lỗi hệ thống (${status}):`, error.response.data?.message || error.message);
      }
    } else if (error.request) {
      // Lỗi mạng, không kết nối được tới Server Backend
      console.error('Không thể kết nối đến máy chủ. Vui lòng kiểm tra lại đường truyền mạng hoặc khởi chạy Backend.');
    } else {
